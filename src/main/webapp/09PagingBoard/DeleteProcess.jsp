...
String type = request.getParameter("type");  // 일련번호 얻기
...
dto = dao.selectView(num,type);  // 주어진 일련번호에 해당하는 기존 게시물 얻기

// 로그인된 사용자 ID 얻기
String sessionId = session.getAttribute("CustNo").toString(); 

int delResult = 0;

if (sessionId.equals(dto.getCustno())) {  // 작성자가 본인인지 확인 
    // 작성자가 본인이면...
    dto.setNum(num);
    dto.setType(type);
    ...
    if (delResult == 1) { 
    	...
    	JSFunction.alertLocation("성공적으로 삭제되었습니다.", redirectUrl, out);
